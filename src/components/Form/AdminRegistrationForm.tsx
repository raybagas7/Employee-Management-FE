...
const AdminRegistrationForm = () => {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    const { error, message } =
      await services.postRegisterNewAdmin(registerPayload);

    if (error) {
      toast.error(message);
    } else {
      toast.success("New admin registered you now can login as admin");
      router.push("/login");
    }
  }
  return (
    <Form {...form}>
      ...
      >
        ...
        <Link href={"/login"}>
          <Button className="absolute bottom-3 right-3" type="button">
            Login
          </Button>
        </Link>
      </form>
